...
function query001() {
	...
	if ((Latlon != "") && (stringRequest != "")) {
		...
		if (Source == 3) {
			...
			var successFn = function(resp) {
				...
				if (report_data.length > 0) {
					for ( var i = 0; i < report_data.length; i++) {
						...
						var AtmAdress = report_data[i].Add;
						var d = Distance(TempLat, TempLon, TempLatAtm,
								TempLonAtm);
						var checkSts = false;
						if ((report_data[i].bank.toUpperCase().search(
								stringRequest.toUpperCase()) != -1)
								|| (report_data[i].name.toUpperCase().search(
										stringRequest.toUpperCase()) != -1)) {
							checkSts = true;
						}
			...
			getData("/atm", null, successFn, null);

			// lay du lieu tu google
			infowindow = new google.maps.InfoWindow();
			service = new google.maps.places.PlacesService(map);
				...
				if (report_data.length > 0) {
					for ( var i = 0; i < report_data.length; i++) {
						...
						var d = Distance(TempLat, TempLon, TempLatAtm,
								TempLonAtm);
						var checkSts = false;
						if ((report_data[i].xCompanyName.toUpperCase().search(
								stringRequest.toUpperCase()) != -1)
								|| (report_data[i].xLaixeHoten.toUpperCase()
			...
			getData("/vehicle", null, successFn, null);
		} else {
			// lay du lieu tu google
			infowindow = new google.maps.InfoWindow();
			service = new google.maps.places.PlacesService(map);
